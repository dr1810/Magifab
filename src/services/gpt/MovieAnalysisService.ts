...
export class MovieAnalysisService {
  /** Loads the saved knowledge base, or builds the deterministic demo baseline once. */
  public ensureMovieKnowledge(movie: MovieData): SemanticMovieMemory {
    const existing = semanticMemoryService.get(movie.id)
    return existing?.version === 3 ? existing : this.seedFromMovie(movie)
  }
  ...
  public seedFromMovie(movie: MovieData): SemanticMovieMemory {
    ...
    const memory: SemanticMovieMemory = {
      movieId: movie.id, version: 3, createdAt: new Date().toISOString(),
      characters: movie.scenes.flatMap((scene) => scene.characterList).filter((character, index, all) => all.findIndex((item) => item.id === character.id) === index).map((character) => ({ ...character, traits: [character.emotionalState], aliases: [character.name.toLowerCase(), character.role.toLowerCase()] })),
      ...
      scenes: movie.scenes.map((scene, index, all) => ({
        sceneId: scene.sceneId,
        range: { start: scene.timestamp, end: all[index + 1]?.timestamp ?? Number.MAX_SAFE_INTEGER },
        summary: scene.voiceNarration,
        dialogue: scene.subtitle ? [scene.subtitle] : [],
        emotions: [scene.emotion],
        importantEvents: [scene.causeEffectData.action],
        characterIds: scene.characterList.map((character) => character.id),
        visibleCharacters: scene.characterList.map((character, characterIndex) => ({
          characterId: character.id,
          prominence: /main/i.test(character.role) || characterIndex === 0 ? 'primary' : 'secondary',
          confidence: 'known',
        })),
        objectIds: [`${scene.sceneId}-object`],
        location: /forest/i.test(`${scene.subtitle} ${scene.voiceNarration}`) ? 'Forest' : 'Unknown',
        keyFrameTimestamps: [scene.timestamp],
        accessibilityMetadata: movie.accessibilityTags,
        confusionPoints: scene.prompts.map((prompt) => ({ timestamp: scene.timestamp, reason: prompt.explanation, suggestedQuestion: prompt.question })),
        companionAnchor: scene.companionPosition,
      })),
