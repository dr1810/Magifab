...
export function MovieViewer({ movie, onBack, onOpenAccessibilitySettings = () => undefined }: MovieViewerProps) {
  ...
  useEffect(() => {
    if (sceneState?.companionEnabled) { setPromptOpen(true); setDrawerOpen(true); return }
    setPromptOpen(false)
    setDrawerOpen(false)
    setWidgetOpen(false)
    setAssistantText(sceneState?.conversation.sceneExplanation ?? 'Select a prompt to get a simple explanation for this scene.')
  }, [sceneState?.companionEnabled, sceneState?.conversation.sceneExplanation, sceneState?.sceneId])
  ...
  }, [scene?.companionPosition, sceneState, settings, startPrompt])

  const closeBubbles = useCallback(() => {
    dismissPrompt(); setWidgetOpen(false); setPromptOpen(false); void stopSpeech()
  }, [dismissPrompt])
  ...
  return <main className="movie-experience viewer-page playback-ready">
    <TopBar movie={movieData} onBack={() => { cancelPrompt(); savePlaybackTimestamp(movie, currentTime, duration || totalDuration); onBack() }} onOpenPrompts={() => { if (!sceneState?.companionEnabled) return; setDrawerOpen(false); setPromptOpen(true) }} onOpenDrawer={() => { if (!sceneState?.companionEnabled) return; setPromptOpen(false); setDrawerOpen(true) }} />
    <div className="viewer-layout" style={{ position: 'relative' }}>
      <MoviePlayer movie={movieData} scene={scene} subtitle={narrativeBanner} playing={playing} muted={muted} volume={volume} currentTime={currentTime} totalTime={duration || totalDuration} onPlayToggle={() => setPlaying((value) => !value)} onMuteToggle={() => setMuted((value) => !value)} onVolumeChange={setVolume} onSeek={(timestamp) => applyTime(timestamp, true)} onTimeChange={applyTime} onDurationChange={setDuration} onSeeking={closeBubbles} onSeekComplete={(timestamp) => applyTime(timestamp, true)} promptOpen={promptOpen} onTogglePromptPanel={() => { if (!sceneState?.companionEnabled) return; setDrawerOpen(false); setPromptOpen((open) => !open) }} onOpenVisualDrawer={() => { if (!sceneState?.companionEnabled) return; setPromptOpen(false); setDrawerOpen(true) }} onOpenPromptPanel={() => { if (!sceneState?.companionEnabled) return; setDrawerOpen(false); setPromptOpen(true) }} onCloseOverlays={closeBubbles} onOpenAccessibilitySettings={onOpenAccessibilitySettings} onCloseBubbles={closeBubbles} reduceMotion={settings.reduceMotion || settings.disableAnimations}
        overlays={<><FloatingBubble content={activeBubble} theme={movieData.companionTheme} reduceMotion={settings.reduceMotion || settings.disableAnimations} visible={bubbleVisible} onOpenCompanion={() => setWidgetOpen(true)} onClose={dismissPrompt} /><CompanionWidget open={widgetOpen} name={profile?.name || 'Lumi'} message={assistantText} theme={movieData.companionTheme} onClose={() => setWidgetOpen(false)} reduceMotion={settings.reduceMotion || settings.disableAnimations} /><PromptPanel open={promptOpen} prompts={prompts} selectedPromptId={selectedPrompt?.id ?? ''} onSelectPrompt={selectPrompt} onClose={() => setPromptOpen(false)} /></>}
        drawerOverlay={<VisualDrawer open={drawerOpen} sceneState={sceneState} onClose={closeDrawer} />}
      />
    </div>
  </main>
}
