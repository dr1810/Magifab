...
try {
  ...
  const { createStoryCompanionPromptContext } = await server.ssrLoadModule('/src/services/narrative/StoryCompanionNavigation.ts')
  ...
  const rexPrompt = conversation?.promptBubbles.find((prompt) => prompt.question === 'Why is Rex annoyed with Ellie?')
  assert.ok(rexPrompt)
  const drawerContext = createStoryCompanionPromptContext(rexPrompt, 'Rex is frustrated because Ellie keeps stopping to study animals while he wants the group to continue.', conversation)
  assert.equal(drawerContext.tab, 'characters')
  assert.equal(drawerContext.question, 'Why is Rex annoyed with Ellie?')
