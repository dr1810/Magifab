...
class DrawerStateBuilder:
    def build(self, state: StoryState, start_timestamp: float) -> TimelineDrawerState:
        return TimelineDrawerState(
            ...
            story_so_far=[event.summary for event in state.story_so_far[-20:]],
            ...
            unresolved_questions=[event.summary for event in state.open_story_threads],
            important_memories=[event.summary for event in state.memory_reminders[-8:]],
        )
